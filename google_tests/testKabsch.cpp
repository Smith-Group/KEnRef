...
TEST(KabschTestSuite, TestTranslate) {
    ...
    auto A_calculated = Kabsch_Umeyama<float>::find3DAffineTransform(after, before, false, true, true);
...
TEST(KabschTestSuite, TestRotate) {
    ...
    auto A_predicted = Kabsch_Umeyama<float>::find3DAffineTransform(after, before, false, true, true);
    std::cout << "A_applied\n" << A_applied.matrix() << std::endl;
...
//    Eigen::Vector3f axis(0, 0, 1);
    ...
    A_predicted = Kabsch_Umeyama<float>::find3DAffineTransform(after, before, false, true, true);
    std::cout << "A_applied\n" << A_applied.matrix() << std::endl;
//    std::cout << "A_applied.matrix().inverse()\n" << A_applied.matrix().inverse() << std::endl;
    std::cout << "A_predicted\n" << A_predicted.matrix() << std::endl;
    std::cout << "difference\n" << (A_predicted.matrix() - A_applied.matrix()) << std::endl;
...
TEST(KabschTestSuite, TestComplexTransform){
    ...
    after = Kabsch_Umeyama<float>::applyTransform(A_applied, after);
    before = Kabsch_Umeyama<float>::translateCenterOfMassToOrigin(before);
    A_predicted = Kabsch_Umeyama<float>::find3DAffineTransform(after, before, true, true, true);
    //TODO complete
}
...
TEST(KabschTestSuite, TestKabschRoundTrip) {
    CoordsMatrixType<float> after1(8, 3), after2(8, 3), before(8, 3);
    before << 0, 0, 0,
            10, 0, 0,
            10, 10, 0,
            0, 10, 0,
    ...
    std::cout << "Q before\n" << after1.matrix() << std::endl;
    after1 = Kabsch_Umeyama<float>::applyTransform(A_applied, after1);
    std::cout << "Q after going\n" << after1.matrix() << std::endl;

    auto A_predicted = Kabsch_Umeyama<float>::find3DAffineTransform(after1, before, false, true, true);
    std::cout << "A_applied\n" << A_applied.matrix() << std::endl;
    std::cout << "A_predicted\n" << A_predicted.matrix() << std::endl;
...
TEST(KabschTestSuite, TestMaxError) {
    ...
    std::cout << "scale\n" << scale << std::endl;
    std::cout << "R\n" << R << std::endl;
    Eigen::Transform<double, 3, Eigen::Affine> A_predicted_d = Kabsch_Umeyama<double>::find3DAffineTransform(out, in,
                                                                                                             false,
                                                                                                             true,
                                                                                                             true);
    ...
    Eigen::Transform<float, 3, Eigen::Affine> A_f = Kabsch_Umeyama<float>::find3DAffineTransform(out.cast<float>(),
                                                                                                 in.cast<float>(),
                                                                                                 false, true, true);
